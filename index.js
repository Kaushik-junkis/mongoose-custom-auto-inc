...
exports.plugin = function (schema, options) {
  ...
  if (!counterSchema || !IdentityCounter) throw new Error("mongoose-custom-auto-inc has not been initialized");
  ...
  var incrementModelCount = function(callback){
	  IdentityCounter.findOneAndUpdate(
	  { model: settings.model, field: settings.field },
	  { $inc: { count: settings.incrementBy } },
	  // { new: true }, // new: true specifies that the callback should get
		// the updated counter.
	  function (err, counter) {
	  if (err) return callback(err);
	  callback(null, counter === null ? settings.startAt : counter.count + settings.incrementBy);
	  ...
	  schema.method('incrementModelCount', incrementModelCount);
	  schema.static('incrementModelCount', incrementModelCount);
